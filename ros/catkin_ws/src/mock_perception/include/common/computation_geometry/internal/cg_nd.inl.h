...
namespace cg {
                                     ...
                                     const T& distance)
   {
      return project_point_t
             (
               source_point,
               destination_point,
               distance / cg::distance(source_point, destination_point)
             );
   }
   ...
   {
      cg::segment<T,D> _segment;

      for (std::size_t i = 0; i < cg::segment<T,D>::PointCount; ++i)
      {
   ...
   template <typename T, std::size_t D>
   inline line<T,D> mirror(const line<T,D>& line, const cg::line<T,D>& mirror_axis)
   {
      cg::line<T,D> _line;

      for (std::size_t i = 0; i < cg::line<T,D>::PointCount; ++i)
      {
   ...
   inline box<T,D> mirror(const box<T,D>& box, const line<T,D>& mirror_axis)
   {
      ...
      for (std::size_t i = 0; i < cg::box<T,D>::PointCount; ++i)
      {
   ...
   {
      cg::triangle<T,D> _triangle;

      for (std::size_t i = 0; i < cg::triangle<T,D>::PointCount; ++i)
      {
   ...
   {
      cg::quadix<T,D> _quadix;

      for (std::size_t i = 0; i < cg::quadix<T,D>::PointCount; ++i)
      {
   ...
   {
      cg::pointnd<T,D> _point = closest_point_on_line_from_point(axis,point);
   ...
   inline segment<T,D> project_onto_axis(const triangle<T,D>& triangle, const line<T,D>& axis)
   {
      ...
      point_list.reserve(cg::triangle<T,D>::PointCount);
   ...
   inline segment<T,D> project_onto_axis(const box<T,D>& box, const line<T,D>& axis)
   {
      ...
      point_list.reserve(cg::box<T,D>::PointCount);
   ...
   inline segment<T,D> project_onto_axis(const quadix<T,D>& quadix, const line<T,D>& axis)
   {
      ...
      point_list.reserve(cg::quadix<T,D>::PointCount);
   ...
   inline polygon<T,D> make_polygon(const triangle<T,D>& triangle)
   {
      ...
      polygon.reserve(cg::triangle<T,D>::PointCount);
   ...
   inline polygon<T,D> make_polygon(const quadix<T,D>& quadix)
   {
      ...
      polygon.reserve(cg::quadix<T,D>::PointCount);
