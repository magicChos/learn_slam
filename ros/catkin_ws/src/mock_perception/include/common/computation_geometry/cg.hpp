...
#ifndef COMPUTATIONAL_GEOMETRY
#define COMPUTATIONAL_GEOMETRY
...
namespace cg {
   ...
   static const char VERSION_INFORMATION[] = "CG Version 0.0.5";
   ...
   template <typename T> inline bool is_right_triangle(const cg::triangle<T,2>& triangle);
   template <typename T> inline bool is_right_triangle(const cg::triangle<T,3>& triangle);
   ...
   template <typename T> inline line<T,2> mirror(const line<T,2>& line, const cg::line<T,2>& mirror_axis);
   ...
   template <typename T> inline line<T,3> mirror(const line<T,3>& line, const cg::line<T,3>& mirror_axis);
...
#include "ace/common/computation_geometry/internal/cg.inl.h"

#endif // COMPUTATIONAL_GEOMETRY
