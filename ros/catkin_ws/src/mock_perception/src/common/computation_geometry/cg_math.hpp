...
#ifndef COMPUTATIONAL_GEOMETRY_MATH
#define COMPUTATIONAL_GEOMETRY_MATH

#include <cmath>
#include <limits>

namespace cg {
    #define CG_DOUBLE_PRECISION

    #ifdef CG_SINGLE_PRECISION
        typedef float Float;
    #endif

    #ifdef CG_DOUBLE_PRECISION
        typedef double Float;
    #endif

    #ifdef CG_EXTENDED_PRECISION
        typedef long double Float;
    #endif

    /**
     * @brief [ Epsilon constants ]
     */
    ...
    /**
     * @brief [ Random resolution constants ]
     */
    ...
    inline T approx_sin(T angle)
    {
        ...
        if ((angle <= T(180.0)) && (angle >     90.0)) {
            angle = T(180.0) - angle;    final_sign = T( 1.0);
        }
        else if ((angle <= T(270.0)) && (angle > T(180.0))) {
            angle = angle    - T(180.0); final_sign = T(-1.0);
        }
        else if ((angle <= T(360.0)) && (angle > T(270.0))) {
            angle = T(360.0) - angle;    final_sign = T(-1.0);
        }
...
#endif // COMPUTATIONAL_GEOMETRY_MATH
