...
#ifndef COMPUTATIONAL_GEOMETRY_INSTANTIATE
#define COMPUTATIONAL_GEOMETRY_INSTANTIATE

#include "ace/common/computation_geometry/cg.hpp"
#include "ace/common/computation_geometry/cg_nd.hpp"
#include "ace/common/computation_geometry/cg_algorithm.hpp"
#include "ace/common/computation_geometry/cg_utilities.hpp"

namespace cg {
   #define INSTANTIATE_CG(T,InputIterator2d,InputIterator3d,OutputIterator2d,OutputIterator3d)\
      ...
      template void create_right_triangle<T>(const cg::point2d<T>& p1, const cg::point2d<T>& p2, cg::point2d<T>& c1, cg::point2d<T>& c2);\
      ...
      template bool is_right_triangle<T>(const cg::triangle<T,2>& triangle);\
      template bool is_right_triangle<T>(const cg::triangle<T,3>& triangle);\
      ...
      template line<T,2> mirror<T>(const line<T,2>& line, const cg::line<T,2>& mirror_axis);\
      ...
      template line<T,3> mirror<T>(const line<T,3>& line, const cg::line<T,3>& mirror_axis);\
   ...
   #define INSTANTIATE_CG_ND(T, D, OutputIterator)\
      ...
      template pointnd<T,D> closest_point_on_segment_from_point<T,D>(const segment<T,D>& segment, const cg::segment<T,D>::PointType& point);\
      template pointnd<T,D> closest_point_on_line_from_point<T,D>(const line<T,D>& segment, const cg::line<T,D>::PointType& point);\
      template pointnd<T,D> closest_point_on_sphere_from_point<T,D>(const hypersphere<T,D>& sphere, const cg::hypersphere<T,D>::PointType& point);\
      template pointnd<T,D> closest_point_on_plane_from_point<T,D>(const plane<T,D>& plane, const cg::plane<T,D>::PointType& point);\
      template pointnd<T,D> closest_point_on_box_from_point<T,D>(const box<T,D>& box, const cg::box<T,D>::PointType& point);\
      ...
      template line<T,D> mirror<T,D>(const line<T,D>& line, const cg::line<T,D>& mirror_axis);\
   ...
   #define INSTANTIATE_CG_UTILITIES_1(T)                                                 \
   ...
   #define INSTANTIATE_CG_UTILITIES_2(T,D)                                                 \
   ...
   #define INSTANTIATE_CG_ALGORITHMS(T,K)                                                                                                \
   ...
   typedef cg::point2d<float>*  flt_pnt_2d;
   typedef cg::point2d<double>* dbl_pnt_2d;

   typedef cg::point3d<float>*  flt_pnt_3d;
   typedef cg::point3d<double>* dbl_pnt_3d;

   INSTANTIATE_CG( float, flt_pnt_2d, flt_pnt_3d, flt_pnt_2d, flt_pnt_3d)
   INSTANTIATE_CG(double, dbl_pnt_2d, dbl_pnt_3d, dbl_pnt_2d, dbl_pnt_3d)

   INSTANTIATE_CG_MATH(float)
   INSTANTIATE_CG_MATH(double)

   INSTANTIATE_CG_UTILITIES_1(float)
   INSTANTIATE_CG_UTILITIES_1(double)

   INSTANTIATE_CG_UTILITIES_2(float,2)
   INSTANTIATE_CG_UTILITIES_2(double,2)

   INSTANTIATE_CG_UTILITIES_2(float,3)
   INSTANTIATE_CG_UTILITIES_2(double,3)

//   INSTANTIATE_CG_ALGORITHMS(float, f)
//   INSTANTIATE_CG_ALGORITHMS(double,d)

} // namespace cg


#endif // COMPUTATIONAL_GEOMETRY_INSTANTIATE
